...
class DataStream(QtWidgets.QWidget):
    ...
    def __init__(self, data_src, parent=None):
        super().__init__(parent)
        self.fish = np.load(data_src)

        self.frame        =  0
        self.frame_buffer = 30
        self.fps          = 30
        self.nfish        = len(self.fish)
        self.nframes      = self.fish[0].n_entries()

        # note: can replace this with time from the data frame
        self.t = np.arange(0,self.nframes/self.fps,1./self.fps)
    ...
    def plot_curves(self):
        ...
        for i in range(self.nfish):
          self.curve1 = self.p1.plot(self.t,self.fish[i].df.dw,pen=pg.mkPen(color[i],width=1))
          self.curve2 = self.p2.plot(self.t,self.fish[i].df.speed,pen=pg.mkPen(color[i],width=1))
          self.curve3 = self.p3.plot(self.t,self.fish[i].df.omega,pen=pg.mkPen(color[i],width=1))
...
class MainWidget(QtWidgets.QWidget):
    def __init__(self, video_src, data_src, parent=None):
        ...
        self.time_box.editingFinished.connect(self.time_box_edit)
    ...
    def t_hhmmsscs(self):
        ...
        ss = ( t_cs / 1e2 ) % 60 
        mm = ( t_cs / ( 1e2 * 60) ) % 60 
        hh = ( t_cs / ( 1e2 * 60 * 60 ) ) % 24 
        return "%02i:%02i:%02i.%02i" % (hh,mm,ss,cs)

    def hhmmsscs_to_frame(self,t_str):
        t_str_split = t_str.split(":")
        ss = mm = hh = ""
        if len(t_str_split) > 0: ss = t_str.split(':')[-1]
        if len(t_str_split) > 1: mm = t_str.split(':')[-2]
        if len(t_str_split) > 2: hh = t_str.split(':')[-3]
        t_cs = 0 
        try:
            if len(ss) != 0: 
                t_cs += float(ss)*   100
    ...
    def time_box_edit(self):
        frame_tmp = self.hhmmsscs_to_frame(self.time_box.text())
        if frame_tmp < self.data_stream.nframes and frame_tmp > -1: 
          self.slide(frame_tmp)
        else:
          self.time_box.setText(self.t_hhmmsscs())
...
def main():
    ...
    if args.datafile:
        ...
        subdir = video_src.split('/')[-1].split('.')[0] + "_kinematics.npy"
