...
def arg_parse():
    ...
    parser.add_argument("-npix","--n_pixel_blur", type=int, help="square-root of n-pixels for threshold blurring", default=3)
    ...
    parser.add_argument("-MOG2", help="switch to mean adaptive method instead of Gaussian", action='store_true')
...
for i_frame in range(cvt.frame_start, cvt.frame_end + 1):
    ...
    if cvt.get_frame():
        ...
        cvt.mask_tank()
        # then use MOG2 background subtraction if option selected
        if args.MOG2:
            cvt.mask_background()
